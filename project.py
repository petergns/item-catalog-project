...
def showCategories():
	...
	# Get lastest category characters added
	catChars = session.query(CatChar).all()
	return render_template('categories.html', categories = categories, catChars = catChars)
...
def showCategory(catalog_id):
	...
	catCharsCount = session.query(CatChar).filter_by(category_id = catalog_id).count()
	return render_template('category.html', categories = categories, catChars = catChars, categoryName = categoryName, catCharsCount = catCharsCount)
...
def showCatChar(catalog_id, char_id):
	...
	return render_template('catChar.html', catChar = catChar, creator = creator)
...
def addCatChar():
	...
	if request.method == 'POST':
		...
		if not request.form['description']:
			...
			return redirect(url_for('addCatChar'))
		# Add category char
		newCatChar = CatChar(name = request.form['name'], description = request.form['description'], category_id = request.form['category'], user_id = login_session['user_id'])
...
@app.route('/catalog/<int:catalog_id>/chars/<int:char_id>/edit', methods=['GET', 'POST'])
def editCatChar(catalog_id, char_id):
	...
	if request.method == 'POST':
		if request.form['name']:
			CatChar.name = request.form['name']
		if request.form['description']:
			CatChar.description = request.form['description']
		if request.form['category']:
			CatChar.category_id = request.form['category']
		return redirect(url_for('showCatChar', catalog_id = catChar.category_id ,char_id = catChar.id))
	else:
		return render_template('editCatChar.html', categories = categories, catChar = catChar)

@app.route('/catalog/<int:catalog_id>/chars/<int:char_id>/delete', methods=['GET', 'POST'])
def deleteCatChar(catalog_id, char_id):
	...
	if request.method == 'POST':
		...
		session.commit()
		return redirect(url_for('showCategory', catalog_id = catChar.category_id))
	else:
